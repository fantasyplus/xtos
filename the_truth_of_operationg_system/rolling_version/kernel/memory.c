...
//  所以让位图可以支持管理512MB的内存空间，所以预留位图大小为16KB，也就是4页，所以选择0x9a000作为位图的起始地址（0x9a000-0x9e000），0x9e000-0x9f000是给内核主线程PCB用的
...
// 返回地址的高10位，即pde索引部分，用来在页目录表定位某一个页目录表项
#define PDE_IDX(addr) ((addr & 0xffc00000) >> 22)
// 返回地址的中间10位，即pte索引部分，用来在页表中定位某一个页表项
...
static void page_table_add(void *_vaddr, void *_page_phyaddr)
{
   ...
   uint32_t *pde = pde_ptr(vaddr); // 获得指向该虚拟地址对应的-页目录表项-的物理地址的指针（指针本身为虚拟地址）
   uint32_t *pte = pte_ptr(vaddr); // 获得指向该虚拟地址对应的-页表项-的物理地址的指针（指针本身为虚拟地址）
   //cpu处理上面的两个指针，经过保护模式对页表的标准处理后，最终却指向了该虚拟地址对应的页目录表项或页表项的物理地址，
   //本质上是因为当时把最后一个页目录表项（1023）指向页目录表开头地址，经过pde_ptr和pte_ptr两个函数的操作
   //达到欺骗cpu的目的（详细见书p392页,很复杂,核心思想是让cpu把页目录表和页表当成正常的页用，从而在页目录表和页表中索引）
   ...
   { // 页目录项不存在(因为新增的虚拟地址超过了页目录表中已分配的页目录表项所描述的范围),所以要先创建页目录项再创建页表项.
      /* 页表中用到的页框一律从内核空间分配 */
      ...
      *pde = (pde_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
      ...
      //就是新申请一页物理内存当作页表，需要先清理一下这块物理内存的内容
...
void mem_init()
{
   ...
   // 在loader.S中调用BIOS方法获得的内存总量(total_mem_bytes)，放在了0xb00开头的dd大小的空间中
